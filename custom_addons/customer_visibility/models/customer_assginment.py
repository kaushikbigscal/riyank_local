...
class CustomerAssignment(models.Model):
    ...
    customer_class_filter = fields.Many2many('customer.class', 'customer_assignment_class_rel', 'assignment_id','class_id', string='Customer Class')
    brand_filter = fields.Many2many('product.brand', 'customer_assignment_brand_rel', 'assignment_id', 'brand_id', string='Brand')
    state_filter = fields.Many2many('res.country.state', 'customer_assignment_state_rel', 'assignment_id', 'state_id', string='State')
    city_filter = fields.Many2many('res.city', 'customer_assignment_city_rel', 'assignment_id', 'city_id', string='City')
    department_filter = fields.Many2many('hr.department', 'customer_assignment_dept_rel', 'assignment_id','department_id', string='Department')
    state_filter_emp = fields.Many2many('res.country.state', 'customer_assignment_emp_state_rel', 'assignment_id','state_id', string='State')
    city_filter_emp = fields.Many2many('res.city', 'customer_assignment_emp_city_rel', 'assignment_id', 'city_id', string='City')
    name = fields.Char(string='Assignment Name', store=True)
    customer_ids = fields.Many2many('res.partner', 'customer_assignment_partner_rel', 'assignment_id', 'partner_id', string='Customers', domain=[('parent_id', '=', False)], context={'show_customer_address': True})
    employee_ids = fields.Many2many('hr.employee', 'customer_assignment_employee_rel', 'assignment_id', 'employee_ids', string='Employees')
...
class CustomerEmployeeReport(models.Model):
    ...
    assignment_id = fields.Many2many('customer.assignment', string='Select Assignments', help="Select assignments to view")
