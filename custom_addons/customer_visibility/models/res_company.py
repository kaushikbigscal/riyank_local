...
class HREmployee(models.Model):
    ...
    customer_access_scope = fields.Selection([('all', 'All'), ('self', 'Self'), ('assigned', 'Assigned'), ('select', 'Select')], string="Customer Access Scope", default='self', required=True)
    allowed_customer_ids = fields.Many2many('res.partner', 'employee_customer_rel', 'employee_id', 'customer_id', string="Allowed Customers")
    has_user = fields.Boolean(string="Has User Account", compute='_compute_has_user', store=True)
    # Location Restrictions
    restrict_by_location = fields.Boolean(string="Restrict By Location", default=False, help="Enable location-based restrictions")
    ...
    allowed_zone_ids = fields.Many2many('zone.master','employee_zone_rel', 'employee_id','zone_id', string="Allowed Zones")
    # Location Selection Fields
    allowed_sub_zone_ids = fields.Many2many('sub.zone','employee_sub_zone_rel','employee_id','zone_master_id', string="Allowed Sub-Zones")
    allowed_state_ids = fields.Many2many('res.country.state','employee_state_rel','employee_id','state_id', string="Allowed States")
    allowed_city_ids = fields.Many2many('res.city', 'employee_city_rel','employee_id','city_id', string="Allowed Cities",domain = "[('state_id', 'in', allowed_state_ids)]")
    ...
    allowed_customer_class_ids = fields.Many2many('customer.class', 'employee_customer_class_rel','employee_id','customer_class_id', string="Allowed Customer Classes")
    allowed_product_brand_ids = fields.Many2many('product.brand','employee_product_brand_rel','employee_id', 'brand_id', string="Allowed Product Brands")
    # Helper for visibility
...
class ResCompany(models.Model):
    ...
    def write(self, vals):
        ...
        for company in self:
            ...
            clear_data = {
                'restrict_zone': {'location_restriction_zone': False, 'allowed_zone_ids': [(5, 0, 0)], 'allowed_sub_zone_ids': [(5, 0, 0)]},
...
class ResPartner(models.Model):
    ...
    created_by = fields.Many2one('hr.employee', string='Created By', default=lambda self: self.env.user.employee_id.id if self.env.user.employee_id else False, readonly=True)
    customer_visibility_access = fields.Many2one('hr.employee', string='Customer Visibility Access', default=lambda self: self.env.user.employee_id.id if self.env.user.employee_id else False)
...
class ProjectTask(models.Model):
    ...
    partner_id = fields.Many2one('res.partner', string="Customer", domain=lambda self: self._get_partner_domain(), context={'show_customer_address': True})
