...
class ResPartner(models.Model):
    ...
    def action_view_customer_call_history(self):
        ...
        return {
            ...
            'context': {'default_task_id': self.id, 'create': False, 'group_by': 'stage_id'},
        }
...
class ResUsers(models.Model):
    ...
    def write(self, vals):
        for record in self:
            ...
            if (checkin or checkout) and company.allowed_distance_service <= 0:
                raise AccessError(_("Allowed distance must be configured in company settings."))

        return super().write(vals)

    def _get_users_on_leave(self):
        """Helper method to get users currently on leave"""
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        ...
        if fsm_mode:
            # Add users on leave filter to args before calling search
            users_on_leave = self._get_users_on_leave()
            if users_on_leave:
                args.append(('id', 'not in', users_on_leave))
    ...
    def web_search_read(self, domain=None, specification=None, offset=0, limit=None, order=None, count_limit=None):
        """Override web_search_read to filter out users on leave in FSM mode"""
        context = self._context or {}
        fsm_mode = context.get('fsm_mode', False)

        if fsm_mode:
            # Add users on leave filter to domain
            users_on_leave = self._get_users_on_leave()
            if users_on_leave:
                domain = domain or []
                domain.append(('id', 'not in', users_on_leave))

        # Try different method signatures based on Odoo version
        try:
            # Try with all parameters first
            return super(ResUsers, self).web_search_read(
                domain=domain,
            ...
            try:
                ...
                try:
                    ...
                    return super(ResUsers, self).web_search_read(domain=domain)

    @api.model
    def search_read(self, domain=None, fields=None, offset=0, limit=None, order=None):
        """Override search_read to filter out users on leave in FSM mode"""
        context = self._context or {}
        fsm_mode = context.get('fsm_mode', False)
        ...
        return super(ResUsers, self).search_read(
            domain=domain,
            fields=fields,
            offset=offset,
            limit=limit,
