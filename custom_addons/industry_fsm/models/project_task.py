...
from odoo.tools import get_lang, json, format_datetime, Markup
...
class Task(models.Model):
    ...
    is_installation_wizard = fields.Boolean("Installation")
    ...
    def default_get(self, fields_list):
        ...
        if 'project_id' in fields_list and not result.get('project_id') and is_fsm_mode:
            ...
            if fsm_project:
                ...
                result["date_deadline"] = datetime.now() + timedelta(hours=1)
    ...
    def _update_deadline(self):
        for task in self:
            if task.planned_date_begin:
                task.date_deadline = task.planned_date_begin + timedelta(hours=1)
    ...
    def create(self, vals):
        ...
        if self._context.get('fsm_mode'):
            ...
            if self._context.get('fsm_mode'):
                if user.has_group('industry_fsm.group_fsm_user') and \
                        ...
                        not user.has_group('industry_fsm.group_fsm_manager'):
        ...
        if not result.department_id and result.is_fsm:
            result.department_id = self.env['hr.department'].search([('name', '=', 'Service Division')], limit=1).id
    ...
    remark = fields.Char(comodel_name="res.partner", readonly=True, compute="_compute_partner_remark", string="Customer Remark")
    ...
    def write(self, vals):
        ...
        previous_stages = {task.id: task.stage_id.id for task in self}
        res = super(Task, self_fsm).write(vals)

        # Handle WhatsApp notifications for stage changes
        if 'stage_id' in vals:
            for task in self:
                prev_stage = previous_stages.get(task.id)
                new_stage_id = task.stage_id.id  # after write
                if prev_stage != new_stage_id and task.is_fsm:
...
#---- Whatsapp code----
    ...
    def send_whatsapp_notification(self, whatsapp_template):
        ...
        # Add call coordinator mobile number if available
        if self.call_coordinator_id and self.call_coordinator_id.mobile:
            phone_numbers.append(self.call_coordinator_id.mobile)
            print(f"? Sending WhatsApp to Call Coordinator: {self.call_coordinator_id.mobile}")

        # Add customer mobile number if available
        if self.partner_id.mobile:
            phone_numbers.append(self.partner_id.mobile)
            print(f"? Sending WhatsApp to Customer: {self.partner_id.mobile}")

        # Check if we have any numbers to notify
        ...
        # if ('{{task_name}}' in formatted_message or '{{stage_name}}' in formatted_message or '{{ticket_number}}' in formatted_message
        #         or '{{ticket_number}}' in formatted_message):
        ...
        #     formatted_message = formatted_message.replace('{{ticket_number}}', self.sequence_fsm or '')

        if (
                '{{task_name}}' in formatted_message
                or '{{stage_name}}' in formatted_message
                or '{{ticket_number}}' in formatted_message
                or '{{assignee_name}}' in formatted_message
                or '{{assignee_number}}' in formatted_message
                or '{{customer_name}}' in formatted_message
                or '{{planned_date}}' in formatted_message
        ):
            # Handle multiple assignees
            assignee_names = ', '.join(self.user_ids.mapped('name')) if self.user_ids else ''
            assignee_numbers = ', '.join(
                filter(None, self.user_ids.mapped('partner_id.mobile'))) if self.user_ids else ''

            planned_date_str = ''
            if self.planned_date_begin or self.date_deadline:
                start_str = format_datetime(self.env, self.planned_date_begin,
                                            dt_format='dd MMM yyyy HH:mm') if self.planned_date_begin else ''
                end_str = format_datetime(self.env, self.date_deadline,
                                          dt_format='dd MMM yyyy HH:mm') if self.date_deadline else ''
                planned_date_str = f"{start_str} - {end_str}" if start_str and end_str else start_str or end_str

            formatted_message = formatted_message.replace('{{task_name}}', self.name or '')
            formatted_message = formatted_message.replace('{{stage_name}}', self.stage_id.name or '')
            formatted_message = formatted_message.replace('{{ticket_number}}', self.sequence_fsm or '')
            formatted_message = formatted_message.replace('{{assignee_name}}', assignee_names)
            formatted_message = formatted_message.replace('{{assignee_number}}', assignee_numbers)
            formatted_message = formatted_message.replace('{{customer_name}}', self.partner_id.name or '')
            formatted_message = formatted_message.replace('{{planned_date}}', planned_date_str)
        ...
        try:
            ...
            for phone_number in phone_numbers:
                ...
                clean_phone_number = re.sub(r'\+\d{1,3}\s*', '', str(phone_number or '')).replace(" ", "")
                ...
                if whatsapp_template.attachment_ids:
                    ...
                    self._send_whatsapp_with_attachments(
                        ...
                        clean_phone_number, formatted_message,
...
#------------------------------------------------------------------------
    ...
    def _get_call_dynamic_field_values(self):
        dynamic_fields = self.env['dynamic.fields'].search([
            ('model_id.model', '=', self._name), ('invisible_type', '=', 'none')
        ])
        result = []
    ...
    def action_open_end_service_wizard(self):
        ...
        return {
            ...
            'context': {
                ...
                'default_is_installation_wizard' : self.is_installation_wizard
            }
        }
...
#            update_values['dynamic_fields'] = json.dumps(existing_dynamic_data)
        ...
        for rec in self:
            ...
            if not values and call_value:
                rec.task_id.dynamic_info = ""
                continue
            # Start building the combined HTML table
            info_html = "<table class='o_list_view table table-sm table-bordered'>"
            ...
            info_html += "<tr><td colspan='12'><b style='color: #28a745;'>Service Call Custom Fields</b></td></tr>"
            # Add all fields together in one section
            for item in all_values:
                info_html += f"""
                       <tr>
                           <td colspan='3'><b>{item['label']}</b></td>
        ...
        self.task_id.field_service_custom_report()
