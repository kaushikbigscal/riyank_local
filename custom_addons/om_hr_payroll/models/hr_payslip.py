...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        self.env['ir.config_parameter'].sudo().set_param('hr_payroll.allow_payslip_download', self.allow_payslip_download)
    ...
    def get_values(self):
        ...
        res.update(
            allow_payslip_download=self.env['ir.config_parameter'].sudo().get_param('hr_payroll.allow_payslip_download', default=False),
...
class HrPayslip(models.Model):
    ...
    def _compute_allow_payslip_download(self):
        ...
        is_admin = self.env.user.has_group('base.group_system') or self.env.user.has_group('om_hr_payroll.group_hr_payroll_manager')
        print(is_admin)
    ...
    def compute_sheet(self):
        for payslip in self:
            ...
            if not it_declaration_year_record:
                raise ValidationError(_("No IT Declaration Found for Year %s Employee Name: %s Or Its Not Locked Yet") % (financial_year.name, payslip.employee_id.name))
        ...
        for contract in contracts.filtered(lambda contract: contract.resource_calendar_id):
            ...
            while current_date <= end_date:
                ...
                if expected_hours_for_day > 0:
                    ...
                    # ?? Adjust expected hours for both public holidays & leaves
                    ...
                    print(
                        f"Day {date_str}: Expected {expected_hours_for_day}h, Public Holiday {public_holiday_hours_for_day}h, "
                        f"Leave {leave_hours_for_day}h, Adjusted Expected {adjusted_expected_hours_for_day}h, Actual {actual_hours_for_day}h"
                    )
