...
class ResConfigSettings(models.TransientModel):
    ...
    def set_values(self):
        ...
        config.set_param('task_deadline_reminder.reminder_day_in_offset_minutes', str(self.reminder_day_in_offset_minutes))
        config.set_param('task_deadline_reminder.reminder_day_out_offset_minutes', str(self.reminder_day_out_offset_minutes))
    ...
    def get_values(self):
        ...
        config = self.env['ir.config_parameter'].sudo()
        res.update({
            'reminder_day_in_offset_minutes': int(config.get_param('task_deadline_reminder.reminder_day_in_offset_minutes', '0')),
            'reminder_day_out_offset_minutes': int(config.get_param('task_deadline_reminder.reminder_day_out_offset_minutes', '0')),
            'enable_overdue_reminder': config.get_param('task_deadline_reminder.enable_overdue_reminder','False') == 'True',
