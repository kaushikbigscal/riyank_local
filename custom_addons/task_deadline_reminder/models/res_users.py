...
class ResUsers(models.Model):
    ...
    def generate_day_in_out_reminders(self):
        ...
        module_installed = self.env['ir.module.module'].sudo().search_count([
            ('name', '=', 'aptuem_attendance_location'),
            ('state', '=', 'installed')
        ])
        if not module_installed:
            print("Module 'aptuem_attendance_location' not installed. Skipping reminder generation.")
            return  # Exit early if module not installed
        ...
        for user in users:
            ...
            if company.day_in_reminder_enabled and user.day_in_reminder_enabled:
                ...
                has_checked_in = self.env['hr.attendance'].sudo().search_count([
            ...
            if company.day_out_reminder_enabled and user.day_out_reminder_enabled:
                ...
                has_checked_out = self.env['hr.attendance'].sudo().search_count([
                    ('employee_id.user_id', '=', user.id),
                    ('check_out', '>=', datetime.combine(today, time.min)),
                    ('check_out', '<=', datetime.combine(today, time.max)),
                ]) > 0
                ...
                if not has_checked_out:
                    ...
                    if attendances:
                        ...
                        if now_local.replace(second=0, microsecond=0) == reminder_time_local.replace(second=0, microsecond=0):
    ...
    def _send_attendance_reminder(self, title, type_str, reminder_time, hour_from, hour_to):
        ...
        if self.partner_id:
            ...
            print("reminder created in task reminder model.")
