...
class MonthlyAttendanceReportLine(models.TransientModel):
    ...
    def generate_report_lines(self, month, employee_ids=None, department_ids=None):
        ...
        self.search([]).unlink()
        # -------------------------------------------------
        # Step 1: Apply access rules based on user groups
        # -------------------------------------------------
        user = self.env.user
        employee_domain = []
        ...
        if user.has_group("hr_attendance.group_hr_attendance_manager"):
            ...
            employee_domain = []
        elif user.has_group("hr_attendance.group_hr_attendance_team_manager"):
            # Manager → self + full hierarchy (all levels of subordinates)
            if user.employee_id:
                employee_domain = [('id', 'child_of', user.employee_id.id)]
        else:
            # Normal user → only own
            if user.employee_id:
                employee_domain = [('id', '=', user.employee_id.id)]
        ...
        # -------------------------------------------------
        # Step 2: Attendance domain (employees with attendance in period)
        # -------------------------------------------------

        # -------------------------------------------------
        # Step 3: Attendance domain (employees with attendance in period)
        # -------------------------------------------------
        attendance_domain = [
            ('check_in', '>=', start_date),
            ('check_in', '<=', end_date),
        ...
        if employee_domain:
            # Use sudo() to search employees without access restrictions
            attendance_domain.append(('employee_id', 'in', self.env['hr.employee'].sudo().search(employee_domain).ids))

        attendance_employees = self.env['hr.attendance'].sudo().search(attendance_domain).mapped('employee_id')
        ...
        for employee in employees:
            # Use sudo() to access employee fields without restrictions
            employee = employee.sudo()

            # Calendar
            calendar = employee.resource_calendar_id or self.env['resource.calendar'].sudo().search([], limit=1)
            if not calendar:
            ...
            while temp_date <= end_date:
                ...
                day_attendance_records = attendance_records.filtered(
                    lambda r: r.check_in and fields.Datetime.from_string(r.check_in)
                    .replace(tzinfo=pytz.UTC).astimezone(tz_kolkata).date() == actual_date
                )
                ...
                if day_attendance_periods:
                    ...
                    expected_start_dt = tz_kolkata.localize(datetime.combine(
                        actual_date,
                        time(int(day_attendance_periods[0].hour_from),
                             int((day_attendance_periods[0].hour_from % 1) * 60))
                    ))
                    expected_end_dt = tz_kolkata.localize(datetime.combine(
                        actual_date,
                        time(int(day_attendance_periods[-1].hour_to),
                             int((day_attendance_periods[-1].hour_to % 1) * 60))
                    ))
                ...
                leave_records_on_day = self.env['hr.leave'].sudo().search([
                    ('employee_id', '=', employee.id),
                ...
                is_public_holiday = bool(self.env['resource.calendar.leaves'].sudo().search([
                ...
                if is_public_holiday:
                    ...
                    if not is_scheduled_working_day and getattr(employee, 'Allow_work_on_sunday', False):
            ...
            uninformed_leave = sum(1 for day, d in formatted_daily_data.items()
                                   if d.get('status') == 'absent' and d.get('is_scheduled_working_day')
                                   and (start_date + timedelta(days=int(day) - 1)) < today)
