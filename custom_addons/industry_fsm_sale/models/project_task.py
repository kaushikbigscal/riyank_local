    ...
    def _onchange_serial_number(self):
        for rec in self:
            if rec.serial_number:
                # Search for the customer product where serial_number_ids matches the selected serial
                customer_product = self.env['customer.product.mapping'].search([
                    ('serial_number_ids', '=', rec.serial_number.id)
                ], limit=1)

                if customer_product:
                    rec.update({
                        'customer_product_id': customer_product.id,
                        'partner_id': customer_product.customer_id.id,
                    })    
...
#        }
    ...
    def _compute_call_coordinator_phone(self):
        ...
        for task in self:
            phone = filter(None, [task.call_coordinator_id.phone, task.call_coordinator_id.mobile])  # Remove empty values
    ...
    def _compute_task_phone_update(self):
        for task in self:
            combined_phones = ','.join(filter(None, [task.call_coordinator_id.phone, task.call_coordinator_id.mobile]))  # Match format
