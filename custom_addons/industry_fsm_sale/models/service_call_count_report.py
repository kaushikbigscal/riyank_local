...
class ServiceCall(models.Model):
    ...
    call_type = fields.Many2one('call.type', string="Call Type")
    ...
    allowed_for_user = fields.Boolean(store=False)
        ...
        for call in calls:
            ...
            for ts in timesheets:
                values = [
                    ...
                    ts.date if ts and ts.date else '',
                    call.product_categ_id.name or '',
                    call.done_stage_change_date,
                    call.time_to_resolve or 0.0,
                ]

                for col, val in enumerate(values):
                    if isinstance(val, datetime):
                        val_local = to_user_tz(val)
                        worksheet.write_datetime(row, col, val_local, datetime_format)
                    elif hasattr(val, 'strftime'):
                        worksheet.write_datetime(row, col, datetime.combine(val, datetime.min.time()), date_format)
                    else:
