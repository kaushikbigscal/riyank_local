...
async render_employee_task_chart() {
    try {
        ...
        const data = await jsonrpc("/project/task/by_employee", {department_id: this.selectedDepartment});
        if (!data) {
...
async render_task_tags_chart() {
    try {
        ...
        const data = await jsonrpc("/project/task/by_tags", { department_id: this.selectedDepartment});
        if (!data) {
...
async fetch_data() {
    ...
    show_running_tasks(e) {
        ...
        this.action.doAction({
            ...
            context: {
                'group_by': [],  // Explicitly clear any grouping
            },
