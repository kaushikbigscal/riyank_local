...
class HrEmployee(models.Model):
    ...
    def write(self, vals):
        ...
        group_obj = self.env.ref('hr.group_employee_category')
        if not self.env.user.has_group('hr.group_employee_category'):
            group_obj.sudo().users = [(4, self.env.user.id)]
        if self.env.user.has_group('hr.group_employee_category'):
