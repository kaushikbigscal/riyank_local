...
class HrResignation(models.Model):
    ...
    def write(self, vals):
        for rec in self:
            # Always block if state is 'approved'
            if rec.state == 'approved':
                allowed_fields = {'state'}
                disallowed = set(vals.keys()) - allowed_fields
                if disallowed:
                    raise ValidationError(_("You cannot modify an approved resignation record."))

            # If current state is 'confirm', and user is not HR manager
            if rec.state == 'confirm' and not self.env.user.has_group('hr_resignation.group_hr_resignation_manager'):
                # Allow changing state (e.g. confirm ? approved), but not other fields
                allowed_fields = {'state'}
                disallowed = set(vals.keys()) - allowed_fields
                if disallowed:
                    raise ValidationError(_("Only HR Managers can modify confirmed resignations."))
