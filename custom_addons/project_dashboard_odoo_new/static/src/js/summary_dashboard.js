...
export class ProjectDashboard extends Component {
    ...
    initDatePicker() {
        ...
        if (typeof flatpickr !== "undefined") {
            ...
            flatpickr(dateInput, {
                ...
                static: false,
                onReady: () => {
                    // Set initial range when calendar is ready
                    ...
                    this.state.startDate = formatToBackend(startOfMonth);
                    this.state.endDate = formatToBackend(endOfMonth);
                    this.state.selectedDateRange = `${this.state.startDate} to ${this.state.endDate}`;

                    // Now that date is set, fetch data
                    this.filters();
                },
                onChange: (selectedDates) => {
                    if (selectedDates.length === 2) {
                        ...
                        this.state.startDate = formatToBackend(selectedDates[0]);
                        this.state.endDate = formatToBackend(selectedDates[1]);
                        this.state.selectedDateRange = `${this.state.startDate} to ${this.state.endDate}`;
    ...
    async fetch_data() {
        try {
            ...
            console.log("Updated dashboard state:", {
                ...
                not_started: this.state.not_started,
            ...
            this.employeeChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    onClick: (evt, activeElements) => {
                        try {
                            const canvasPosition = Chart.helpers.getRelativePosition(evt, this.employeeChart);
                            ...
                            for (let i = 0; i < labelCoordinates.length; i++) {
                                const { label, x, y } = labelCoordinates[i];
                                const buffer = 10;
                                if (Math.abs(clickX - x) <= buffer && Math.abs(clickY - y) <= buffer) {
                                    this.show_tasks_by_employee(label, filters);
                                    return;
                                }
                            }
    ...
    async render_task_tags_chart() {
        try {
            ...
            let labelCoordinates = []; // Store coordinates of label counts
            ...
            this.tagsChart = new Chart(ctx, {
                ...
                options: {
                    ...
                    onClick: (evt, activeElements) => {
                        try {
                            const canvasPosition = Chart.helpers.getRelativePosition(evt, this.tagsChart);
                            const clickX = canvasPosition.x;
                            const clickY = canvasPosition.y;
                            ...
                            for (let i = 0; i < labelCoordinates.length; i++) {
                                ...
                                const buffer = 10;
                                if (Math.abs(clickX - x) <= buffer && Math.abs(clickY - y) <= buffer) {
                                    this.show_tasks_by_tag(label, filters);
                                    return;
                                }
                            }
        ...
        try {
            ...
            if (!isAdmin && userId) {
                ...
                if (projectIds.length > 0) {
                    ...
                    console.log("User is PM â€” showing tasks from their projects or assigned to them");
                } else {
                    domain.push(['user_ids', 'in', [userId]]);
                    domain.push(['state', 'in', ['01_in_progress', '02_changes_requested', '03_approved', '05_not_started']]);
                    console.log("User is not PM â€” showing only assigned tasks");
                }
            }
        ...
        try {
            ...
            if (isAdmin) {
                ...
                if (projectIds.length > 0) {
                    ...
                    domain.push(['project_id', 'in', projectIds]);
                    domain.push(['user_ids', 'in', [userId]]);
                    domain.push(['state', 'in', ['01_in_progress', '02_changes_requested', '03_approved', '1_done', '1_canceled','04_waiting_normal','04_waiting_for_customer']]);
...
//                    domain.push(['depend_on_ids', '=', false]); 
    ...
    async show_running_tasks(e) {
        ...
        try {
            ...
            let domain = [
                ['state', 'in', ['01_in_progress', '02_changes_requested', '03_approved','04_waiting_normal','04_waiting_for_customer']],
                ['project_id', '!=', false],
                ['active', '=', true],
                ['project_id.is_fsm', '=', false],
                ['project_id.is_project_template', '=', false],
            ...
            const userId = this.user.userId;
             ...
             if (!this.state.has_project_admin_rights) {
                ...
                if (projectIds.length > 0) {
                    ...
                    console.log("User is PM filtered by assigned tasks or managed projects", projectIds.length);
                } else {
                    domain.push(['user_ids', 'in', [userId]]);
                    domain.push(['state','in',['01_in_progress', '02_changes_requested', '03_approved']])
                    console.log("User is not PM of any project only assigned tasks shown", projectIds.length);
                }
            }
            ...
            if (!hasProjects.length) {
                console.warn("No show running tasks found matching filters.", domain);
                return;
            }
    ...
    async show_canceled_task(e) {
        ...
        try {
            ...
            if (this.state.has_project_admin_rights) {
                // Full access for project admins â€” no restriction on projects
                ...
                if (projectIds.length > 0) {
                    ...
                    console.log("Project Manager: Viewing canceled tasks of managed projects");
                }
                else {
    ...
    async show_active_projects(e) {
        ...
        try {
            ...
            if (!userId) {
                console.error("User ID not found â€” cannot proceed.");
                return;
            }
            ...
            if (!isAdmin) {
                ...
                const taskDomain = [['user_ids', 'in', [userId]], 
                ...
                console.log("Admin user â€” showing all filtered active projects:", domain);
            }
    ...
    async show_done_task(e) {
        ...
        try {
            ...
            if (!hasProjects.length) {
                console.warn("No show done tasks found matching filters.");
                return;
            }
    ...
    async show_expired_tasks(e) {
        ...
        try {
            ...
            let taskDomain = [
                ['project_id', 'in', projectIds],
                ['state', 'in', ['01_in_progress', '02_changes_requested', '03_approved','04_waiting_normal','04_waiting_for_customer', '05_not_started']],
                ['active', '=', true],
            ];
            ...
            if (!isAdmin) {
                if (managedProjectIds.length) {
                    ...
                    taskDomain.push(['state', 'in', ['01_in_progress', '02_changes_requested', '03_approved', '05_not_started']])
