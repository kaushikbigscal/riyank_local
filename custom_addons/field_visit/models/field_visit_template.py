...
class FieldVisitTemplate(models.Model):
    ...
    _description = "Field Visit Template"
    ...
    def _get_field_visit_plan_types(self):
        ...
        if company.enable_route_wise:
            selection.append(('beat_wise', 'Beat-wise'))

        # if company.enable_unplanned:
        #     selection.append(('unplanned', 'Unplanned'))
    ...
    def _onchange_zip_code(self):
        if self.zip_code:
            zip_names = self.zip_code.mapped('name')
            partners = self.env['res.partner'].search([('zip', 'in', zip_names)])
    ...
    def _compute_visit_ids_count(self):
        visit_data = self.env["field.visit"].read_group(
    ...
    def _prepare_field_visit_vals(self, dates):
        visit_vals = []
        for date in dates:
    ...
    def _create_visits(self, days=7):
        return self._prepare_field_visit_vals(
            self._get_recurrence_dates(days)
        )

    def create_visits(self, days=7):
        for item in self:
            visits = self.env["field.visit"].create(
                item._create_visits(days)
            )
            if visits and item.auto_validate:
                visits.action_confirm()
            if isinstance(item.last_visit_date, datetime.date):
                if item.last_visit_date >= item._get_max_date():
                    item.state = "done"
            else:
                # Handle the case where last_visit_date is not a valid date
                raise ValidationError(_("The last visit date is not valid."))
