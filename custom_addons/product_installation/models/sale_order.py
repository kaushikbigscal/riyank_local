...
class SaleOrder(models.Model):
    ...
    # def _onchange_secondary_order_distributor(self):
    #     """
    #     When order type is Secondary and distributor is selected:
    #     - Update company_id to distributor's company
    #     """
    #     for order in self:
    #         if order.sale_order_type == 'secondary_orders' and order.distributor_id:
    #             distributor_company = order.distributor_id.company_id
    #             if distributor_company:
    #                 # Force update company_id
    #                 order.company_id = distributor_company.id
    #
    # @api.onchange('sale_order_type', 'distributor_id')
    # def _onchange_primary_order_distributor(self):
    #     for order in self:
    #         if order.sale_order_type == 'primary_orders' and order.distributor_id:
    #             # order.partner_id = order.distributor_id
    #             order.partner_invoice_id = order.distributor_id
    #             order.partner_shipping_id = order.distributor_id
    #         elif order.sale_order_type != 'primary_orders' and order.partner_id:
    ...
    def create(self, vals_list):
        ...
        # if self.env['ir.config_parameter'].sudo().get_param('product_installation.enable_distributor'):
        #     orders._generate_installation_lines()
        #     for order in orders:
        #         # If it's a secondary order → assign distributor's user
        #         if order.sale_order_type == 'secondary_orders':
        #             if order.distributor_id and order.distributor_id.user_id:
        #                 order.user_id = order.distributor_id.user_id
        #             else:
        #                 order.user_id = self.env.user
        #         # If it's a primary order → assign current user (or handle separately if needed)
        #         elif order.sale_order_type == 'primary_orders':
        #             order.user_id = self.env.user
    ...
    def write(self, vals):
        ...
        # # Update user_id based on sale_order_type without recursion
        # if self.env['ir.config_parameter'].sudo().get_param('product_installation.enable_distributor'):
        #     for order in self:
        #         user_to_set = None
        #         if order.sale_order_type == 'secondary_orders':
        #             if order.distributor_id and order.distributor_id.user_id:
        #                 user_to_set = order.distributor_id.user_id.id
        #             else:
        #                 user_to_set = self.env.user.id
        #         elif order.sale_order_type == 'primary_orders':
        #             user_to_set = self.env.user.id
        #
        #         if user_to_set and order.user_id.id != user_to_set:
        #             # Avoid recursion by using super().write()
        #             super(SaleOrder, order).write({'user_id': user_to_set})
    ...
    #     for order in self:
    #         if order.sale_order_type == 'primary_orders' and order.distributor_id:
    #             warehouse = order.distributor_id.warehouse_id
    #             if warehouse and order.picking_ids:
...
class SaleOrderLine(models.Model):
    ...
    # def _compute_is_installation(self):
    #     for line in self:
    #         line.is_installation = line.order_id.sale_order_type != "primary_orders"
    #
    #
    # @api.depends("product_id.installed_ok", "product_template_id.installed_ok")
    # def _compute_is_installation(self):
    #     for line in self:
    #         # Check product first, fallback to template
    #         if line.product_id and line.product_id.installed_ok:
    #             line.is_installation = True
    #         elif line.product_template_id and line.product_template_id.installed_ok:
    #             line.is_installation = True
    #         else:
    #             line.is_installation = False
