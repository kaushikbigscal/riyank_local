...
class InstallationTicketPortal(PortalHomePage):
    ...
    def installation_request(self, task_id, **post):
        ...
        if task and task.project_id.name == 'Service Call' and task.is_fsm:
            ...
            task.sudo().write({'installation_request_confirmed': True})

            # Get all users of the department
            if task.department_id:
                users_in_department = request.env['hr.employee'].sudo().search([
                    ('department_id', '=', task.department_id.id),
                    ('user_id', '!=', False)
                ]).mapped('user_id')
                partners = users_in_department.mapped('partner_id')
                # Send notification
                if partners:
                    task.message_post(
