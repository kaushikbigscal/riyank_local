...
class project_project(models.Model):
    ...
    def new_project(self):
        ...
        for project in self:
            ...
            # Generate a unique project name based on current minutes in IST
            project_name = f"{project.name} - {current_time.strftime('%M')}"

            # Create a new project based on the template with the unique name
            new_project_id = project.with_context(project_template=True).copy({
                'name': project_name,
                'project_project_id': project.id,
                'is_project_template': False
            })

            # Set the template task and sequence state for the new project
            if state_new_id:
                new_project_id.write({
                    'template_task_id': state_new_id.id,
                    'sequence_state': 1
                })
