...
class ConfigurationManager(models.Model):
    ...
    def action_authenticate(self):
        ...
        masked_url = url.replace(self.token, '***')
        ...
        try:
            ...
            raise ValidationError(_("Connection error. Please check your server settings and network connectivity."))
        except requests.Timeout:
            raise ValidationError(_("Connection Timeout: The Server Took So long to Respond. Please Try Again Later"))
        except requests.RequestException as e:
            raise ValidationError(_(f"An error occurred while connecting to the server. Please try again later."))
    ...
    def get_qr_code(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200 and response_data.get('qrcode'):
                ...
                time.sleep(retry_delay)
        raise ValidationError(_("QR Code could not be retrieved. Try Again Letter :)"))
