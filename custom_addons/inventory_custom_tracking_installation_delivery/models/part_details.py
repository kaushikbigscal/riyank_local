...
class ProductMapping(models.Model):
    ...
    def create(self, vals):
        ...
        # Step 1: Auto-create mapping parts from product template parts
            ...
            for part in tmpl.part_ids:
                if part.display_name:
                    self.env['customer.product.mapping.part'].create({
                        ...
                        'product_id': part.display_name.id,  # keep product.template
                    })

        # Step 2: If source is Sale Order → assign serial numbers from Delivery Orders
        if record.source_type == 'sale_order' and record.order_id:
            order_name = record.order_id.order_id.name

            pickings = self.env['stock.picking'].search([('origin', '=', order_name)])
            if pickings:
                for picking in pickings:
                    for part in picking.picking_parts_ids:
                        # Find mapping part by template match
                        mapping_part = record.mapping_parts_ids.filtered(
                            lambda m: m.product_id.id == part.original_product_id.product_tmpl_id.id
                        )

                        if mapping_part and part.lot_ids:
                            for lot in part.lot_ids:
                                # If mapping part has no serial yet → reuse it
                                if not mapping_part.serial_number_ids:
                                    mapping_part.write({
                                        'serial_number_ids': lot.id
                                    })
                                else:
                                    # Otherwise → create a duplicate mapping part for the extra serial
                                    self.env['customer.product.mapping.part'].create({
                                        'mapping_id': record.id,
