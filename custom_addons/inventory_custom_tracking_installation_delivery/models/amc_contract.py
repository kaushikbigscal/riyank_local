...
class AmcContract(models.Model):
    ...
    def _check_stage_change(self):
        for record in self:
            if record.stage_id == 'expired' and record.end_date != date.today():
                raise ValidationError("You can only set the stage to 'Expired' if the end date is today.")
    ...
    def create(self, vals):
        ...
        if not self.env.context.get('skip_sticky_notification') and not record.asset_line_ids:
            record._send_sticky_notification(
                "Warning: Please add at least one product in asset tab!"
            )
        return record
    ...
    def _cron_update_expired_contracts(self):
        ...
        contracts.write({'stage_id': 'expired'})
    ...
    def _compute_is_near_expiry(self):
        ...
        for record in self:
            if record.end_date:
                ...
                # True if within next 30 days OR already expired
                record.is_near_expiry = (0 <= delta <= 30) or (delta < 0)
            else:
    ...
    def write(self, vals):
        for rec in self:
            if rec.stage_id == 'expired':
                raise ValidationError("You cannot modify the contract because it is Expired.")
            ...
            for rec in self:
                ...
                if start_date and start_date > date.today():
                    raise ValidationError(
                        "You cannot activate the contract because the Start Date is in the future."
                    )
